...
export default function SettingsScreen() {
  ...
  const { checkAdminStatus } = useTeamLogoStore();
  
  // Check admin status whenever the API key changes
  useEffect(() => {
    if (apiKey) {
      console.log('Settings screen - checking admin status with key:', apiKey);
      ...
      <View style={styles.section}>
        ...
        <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          ...
          <View style={[styles.settingRow, styles.borderTop, { borderColor: colors.border }]}>
            ...
            <Text style={[styles.versionText, { color: colors.muted }]}>18 Football Leagues</Text>
          </View>
        </View>
